const CHECK_IN_CONFIG = {
  ...
  START_REMIND_TIME: 10, // 签到提醒的开始时间（填写分钟，hours默认为9，不可更改）,即每天9:10开始发送签到提醒
  LATEST_TIME: 48, // 最后的签到时间，在此之前只会发送提醒，并且手动签到后不会再自动签到
};
const CHECK_OUT_CONFIG = {
  ENABLE_REMIND: true, // 是否开启签退提醒，工作时长满8h 后开始提醒，不用设置提醒时间
  REMIND_INTERVAL: 15, // 签到提醒的时间间隔，单位为分钟
  LATEST_TIME: {
...
const WORING_TIME = 7.5; // 工作时长，即在签到时间基础上增加8小时打卡（实际为8上下的随机数并增加1小时的午休时长），优先级低于上面的固定签退时间；
